...
public class Wrapper extends Panel implements Serializable, MouseListener,
		...
		for (Enumeration e = wets.elements(); e.hasMoreElements();) {
			...
			if (wet.targetListener instanceof PropertyHookup) {
				...
				for (Enumeration keys = table.keys(); keys.hasMoreElements();) {
					...
					for (Enumeration ee = targets.elements(); ee
							.hasMoreElements();) {
	...
	synchronized void addEventTarget(String eventSetName,
		...
		if (esd == null) {
			System.err
					.println("Internal error: Wrapper.addEventTarget missing event set");
		...
		if (adder == null || remover == null) {
			System.err
					.println("Internal error: Wrapper.addEventTarget missing add/remote listener");
			System.err.println("        eventSetName = " + eventSetName);
			System.err.println("        bean = " + bean);
			return;
		}
	...
	void removeListeners() {
		Enumeration enu = eventTargets.elements();
	...
	public void paint(Graphics g) {
		if (active && Beans.isDesignTime()) {
			...
			for (int x = 0; x < width; x += hashBarLength) {
				...
				g.drawImage(xHashBar, x + bottomNudge, height - hashBarWidth,
						null);
			}
			...
			for (int y = 0; y < height; y += hashBarLength) {
				...
				g.drawImage(yHashBar, width - hashBarWidth, y + rightNudge,
						null);
			}
	...
	public synchronized void doMouseStuff(MouseEvent evt) {
		...
		boolean onTheBorder = ((evt.getSource() == this) && (x < lowX
				|| x > highX || y < lowY || y > highY));
		...
		if (onTheBorder) {
			if (id == MouseEvent.MOUSE_PRESSED) {
				...
				if (cursor == nwResizeCursor || cursor == swResizeCursor
						|| cursor == neResizeCursor || cursor == seResizeCursor) {
	...
	/**
	 * @deprecated Provided for backwards compatibility only. We support
	 *             old-style event handling as a backward compatibility feature
	 *             when we're wrapping a bean using the old event model.
	 */
	...
	public boolean handleEvent(Event evt) {
		...
		if (evt.id == Event.MOUSE_DOWN) {
			me = new MouseEvent(source, MouseEvent.MOUSE_PRESSED, 0, 0, x, y,
					0, false);
			mousePressed(me);
		} else if (evt.id == Event.MOUSE_UP) {
			if (sawMouseDown) {
				me = new MouseEvent(source, MouseEvent.MOUSE_CLICKED, 0, 0, x,
						y, 0, false);
				mouseClicked(me);
			}
			sawMouseDown = false;
			me = new MouseEvent(source, MouseEvent.MOUSE_RELEASED, 0, 0, x, y,
					0, false);
			mouseReleased(me);
		} else if (evt.id == Event.MOUSE_MOVE) {
			me = new MouseEvent(source, MouseEvent.MOUSE_MOVED, 0, 0, x, y, 0,
					false);
			mouseMoved(me);
		} else if (evt.id == Event.MOUSE_ENTER) {
			sawMouseDown = false;
			me = new MouseEvent(source, MouseEvent.MOUSE_ENTERED, 0, 0, x, y,
					0, false);
			mouseEntered(me);
		} else if (evt.id == Event.MOUSE_EXIT) {
			sawMouseDown = false;
			me = new MouseEvent(source, MouseEvent.MOUSE_EXITED, 0, 0, x, y, 0,
					false);
			mouseExited(me);
		} else if (evt.id == Event.MOUSE_DRAG) {
			me = new MouseEvent(source, MouseEvent.MOUSE_DRAGGED, 0, 0, x, y,
					0, false);
			mouseDragged(me);
		}
		return false;
	...
	void listenForMice(boolean enable) {
		...
		if (!useNewEventModel()) {
			System.err.println("WARNING: \"" + child.getClass().getName()
	...
	private static Cursor moveCursor = Cursor
