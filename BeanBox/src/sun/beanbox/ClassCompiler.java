...
public class ClassCompiler {
	...
	public static boolean compile(String fileName, String classpath) {
		...
		try {
			...
			// + args[3]
			// );
			String cmd = String.format("javac %s \"%s\" %s \"%s\"", args[0],
					args[1], args[2], args[3]);
			p = Runtime.getRuntime().exec(cmd);

			return (p.waitFor() == 0) ? true : false; // status code 0 indicates
														// success.
		} catch (Throwable th) {
			System.err
					.println("WARNING: Could not Runtime.exec(String) \"javac\" in"
							+ " the standard way: " + th);
		}
	...
	static boolean warningMessage() {
		System.err.println("");
		System.err
				.println("Check that the version of \"javac\" that you are running");
		System.err
				.println("is the one supplied with Sun's JDK1.x (which includes the");
		System.err
				.println("compiler classes) and not some other version of \"java\"");
