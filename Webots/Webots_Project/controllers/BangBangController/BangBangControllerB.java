...
public class BangBangControllerB extends DifferentialWheels {
  ...
  public static final int DIST_SENSOR_L = 0; // sensor left
  public static final int DIST_SENSOR_LF = 1; // sensor left front
  public static final int DIST_SENSOR_RF = 2; // sensor right front
  public static final int DIST_SENSOR_R = 3; // sensor right
  public static final int DIST_SENSOR_MAX = 200; // maximum distance for sensors
  ...
  public void run() {
    ...
    while (step(STEP_TIME) != -1) {
      ...
      // avoid crashing into wall
      /*if(distanceSensors[DIST_SENSOR_L].getValue() > DIST_SENSOR_MAX ||
         distanceSensors[DIST_SENSOR_LF].getValue() > DIST_SENSOR_MAX){
        driveRight();
      } else if (distanceSensors[DIST_SENSOR_R].getValue() > DIST_SENSOR_MAX ||
         distanceSensors[DIST_SENSOR_RF].getValue() > DIST_SENSOR_MAX) {
        driveLeft();
      } else {
        driveForward();
      }*/
      ...
      if((distanceSensors[DIST_SENSOR_R].getValue() > DIST_SENSOR_MAX ||
         distanceSensors[DIST_SENSOR_RF].getValue() > DIST_SENSOR_MAX) && 
         (distanceSensors[DIST_SENSOR_L].getValue() > DIST_SENSOR_MAX ||
         distanceSensors[DIST_SENSOR_LF].getValue() > DIST_SENSOR_MAX)) {
        driveStop();
      } else if(getRightLightValue() < getLeftLightValue() + LIGHT_SENSOR_DIFF){
        driveRight();
