...
public class Plot3D extends Canvas implements Runnable {
  ...
  public void paint(Graphics g) {
    ...
    // viewport center
    Point3D vc = new Point3D(0, -150, 0);
    Triangle sortedTriangles[];
    sortedTriangles = sortTriangles(triangles, vc);
    ...
    // Draw triangles in background buffer
    Point3D viewPoint = new Point3D(0, -150, 0);
    drawTriangles(bufferg, sortedTriangles, viewPoint);
  ...
  Point3D vc) {
    ...
    for(int i = 0; i < NTRIANGLES; i++) {
      distance[i] = computeDistance(triangles[i], vc);
    }
  ...
  Triangle sortedTriangles[], Point3D viewPoint) {
    Dimension d = getSize();
    int w = d.width;
    int h = d.height;
