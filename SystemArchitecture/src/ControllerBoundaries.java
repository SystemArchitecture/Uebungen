...
public class ControllerBoundaries implements IController {
	...
	@Override
	public void init() {
		if(!_sensorManager.isActive(Sensor.DIST_SENSOR_L)){
			_sensorManager.initialize(Sensor.DIST_SENSOR_L);
		}
		if(!_sensorManager.isActive(Sensor.DIST_SENSOR_LF)){
			_sensorManager.initialize(Sensor.DIST_SENSOR_LF);
		}
		if(!_sensorManager.isActive(Sensor.DIST_SENSOR_R)){
			_sensorManager.initialize(Sensor.DIST_SENSOR_R);
		}
		if(!_sensorManager.isActive(Sensor.DIST_SENSOR_RF)){
			_sensorManager.initialize(Sensor.DIST_SENSOR_RF);
		}
		if(!_motionManager.isActive(Actor.DIFFERENTIAL_WHEELS)){
