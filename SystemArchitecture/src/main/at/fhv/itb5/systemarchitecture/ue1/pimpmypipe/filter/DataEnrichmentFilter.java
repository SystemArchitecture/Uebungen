...
public abstract class DataEnrichmentFilter<in, out> extends AbstractFilter<in, out> implements Runnable {
	...
	/**
	 * read an entity from the filter. the filter will act like an
	 * passive-filter
	 * @throws EndOfStreamException 
	 */
	public out read() throws StreamCorruptedException, EndOfStreamException {
		// just read the next entity and return it
		return getNextEntity();
	}

	/**
	 * write an entity into the filter. the filter will act like an
	 * passive-filter and passes the entity to the next filter, after it
	 * processed it
	 * 
	 * @param value
		...
		if (!m_EndOfStream) {
			if (value != ENDING_SIGNAL) {
				if (m_TempWriteEntity == null)
					m_TempWriteEntity = getNewEntityObject();
	 ...
	 * @throws EndOfStreamException
	 */
	protected out getNextEntity() throws StreamCorruptedException, EndOfStreamException {
		if (!m_EndOfStream) {
	...
	/**
	 * Abstract Filter class asks implementation for new Entity-Object (because
	 * it doesn't know the constructor-arguemnts)
	 * 
	 * @return
	 */
